        ...
        private void OnClickImportCSV(object sender, EventArgs e)
        {
            ...
            if (dialog.ShowDialog() == DialogResult.OK)
            {
                ...
                using (StreamReader sr = new StreamReader(dialog.FileName))
                {
					...
					while ( (i = sr.Read()) != -1 )
					{
						...
						if(p=='\r' && c=='\n')//cr+lf
						{
							if ( line.Length != 0 && line[0]!='#' )
							{
								...
								int val;
								if(split.Length>1 && int.TryParse(split[0], out val) && val>=0)
								{
									//if(!keyvalues.ContainsKey(val))
										//MessageBox.Show(String.Format("CliLoc CONTAINS LINE {0}", val), "WARNING", MessageBoxButtons.OK, MessageBoxIcon.Exclamation, MessageBoxDefaultButton.Button1);
									keyvalues[val]=split[1];
								}
							}
							line.Clear();
						}
                    ...
                    {
						string val;
						if(keyvalues.TryGetValue(entry.Number, out val))
						{
							entry.Text = val;
							entry.Flag = StringEntry.CliLocFlag.Modified;
							Options.ChangedUltimaClass["CliLoc"] = true;
							keyvalues.Remove(entry.Number);
						}
                    }
					foreach(KeyValuePair<int, string> kvp in keyvalues)
					{
						...
						foreach(StringEntry entry in cliloc.Entries)
                    	{
							if (entry.Number > kvp.Key)
	                        {
	                            cliloc.Entries.Insert(index, new StringEntry(kvp.Key, kvp.Value, StringEntry.CliLocFlag.Custom));
	                            Options.ChangedUltimaClass["CliLoc"] = true;
	                            break;
	                        }
							++index;
